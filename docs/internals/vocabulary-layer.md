...
| `algorithms/clause_predicates.h` | Closed checks of whether a node is an admissible statement-level clause, and of the canonical order clauses must appear in: `select_clause_rank_v`, `is_select_clause`, `is_statement_clause`, `check_select_clause_order_v`. One file across statement kinds, because they all share a single ranking mechanism. Whether a clause holds an expression rather than another clause is not checked here — the clause factories enforce that on the argument they are handed. |
