...
| `algorithms/expression_element_predicates.h` | Closed checks of whether a node is an admissible element of a compound expression construct, sectioned by construct. Today: which end of a window frame a boundary node may occupy — `is_frame_start_bound_v`, `is_frame_end_bound_v`, composed from the boundary grammar traits, since every boundary node is a boundary but not at both ends. Whether an element holds an expression rather than a clause is not checked here — the element factories enforce that on the argument they are handed. |
