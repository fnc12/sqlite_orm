...
//            return t;
    ...
    struct table_impl<H, T...> : private table_impl<T...> {
         ...
         */
        /*std::vector<std::string> column_names(typename std::enable_if<internal::is_column<H>::value>::type * = nullptr) {
...
//            return (const char*)sqlite3_column_text(stmt, columnIndex);
    ...
    struct storage_impl<H, Ts...> : public storage_impl<Ts...> {
        ...
        std::string serialize_column_schema(constraints::foreign_key_t<C, R> fk) {
            ...
            auto refTableName = this->find_table_name<ref_type>();
            auto refColumnName = this->column_name(fk.r);
            ss << "FOREIGN KEY(" << this->column_name(fk.m) << ") REFERENCES ";
//            ss << "artist(artistid)";
        ...
        void create_table(sqlite3 *db, const std::string &tableName) {
            ...
            auto index = 0;
            this->table.for_each_column_with_constraints([columnsCount, &index, &ss, this] (auto c) {
    ...
    protected:
        ...
        template<class F, class O>
        std::string string_from_expression(const F* (O::*g)() const, bool noTableName = false) {
            std::stringstream ss;
            if(!noTableName){
                ss << " '" << this->impl.template find_table_name<O>() << "'.";
            }
        ...
        template<class F, class O>
        std::string string_from_expression(void (O::*s)(F), bool noTableName = false) {
            std::stringstream ss;
            if(!noTableName){
                ss << " '" << this->impl.template find_table_name<O>() << "'.";
            }
...
//                                                   auto member_pointer = c.member_pointer;
        ...
        void drop_table(const std::string &tableName) {
            ...
            this->impl.drop_table(tableName, db);
        }
            ...
            this->impl.for_each([&result, db, preserve](auto impl){
                ...
                if(schema_stat != decltype(schema_stat)::already_in_sync) {
                    if(schema_stat == decltype(schema_stat)::new_table_created) {
                        impl->create_table(db, impl->table.name);
                        res = decltype(res)::new_table_created;
                    }else{
                        if(schema_stat == sync_schema_result::old_columns_removed ||
                           ...
                           schema_stat == sync_schema_result::new_columns_added_and_old_columns_removed) {
                            ...
                            impl->drop_table(impl->table.name, db);
                            impl->create_table(db, impl->table.name);
                            res = decltype(res)::dropped_and_recreated;
                        }
                    }
