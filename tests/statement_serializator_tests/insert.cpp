...
TEST_CASE("statement_serializator insert") {
        ...
        SECTION("2 rows") {
            ...
            SECTION("or abort") {
                auto statement = insert(or_abort(),
                                        ...
                                        values(std::make_tuple(1, "The Weeknd"), std::make_tuple(4, "Jonas Blue")));
                value = serialize(statement, context);
                expected = "INSERT OR ABORT INTO users (\"id\", \"name\") VALUES (1, 'The Weeknd'), (4, 'Jonas Blue')";
            }
            SECTION("or fail") {
                auto statement = insert(or_fail(),
                                        into<User>(),
                                        ...
                                        values(std::make_tuple(1, "The Weeknd"), std::make_tuple(4, "Jonas Blue")));
                value = serialize(statement, context);
                expected = "INSERT OR FAIL INTO users (\"id\", \"name\") VALUES (1, 'The Weeknd'), (4, 'Jonas Blue')";
            }
            SECTION("or ignore") {
                auto statement = insert(or_ignore(),
                                        into<User>(),
                                        ...
                                        values(std::make_tuple(1, "The Weeknd"), std::make_tuple(4, "Jonas Blue")));
                value = serialize(statement, context);
                expected = "INSERT OR IGNORE INTO users (\"id\", \"name\") VALUES (1, 'The Weeknd'), (4, 'Jonas Blue')";
            }
            SECTION("or replace") {
                auto statement = insert(or_replace(),
                                        into<User>(),
    ...
    SECTION("select") {
        ...
        SECTION("or replace") {
            auto statement = insert(or_replace(), into<User>(), select(columns(&UserBackup::id, &UserBackup::name)));
            value = serialize(statement, context);
            expected = "INSERT OR REPLACE INTO users SELECT \"users_backup\".\"id\", \"users_backup\".\"name\" FROM "
                       "'users_backup'";
        }
        SECTION("or rollback") {
            auto statement = insert(or_rollback(), into<User>(), select(columns(&UserBackup::id, &UserBackup::name)));
            value = serialize(statement, context);
            expected = "INSERT OR ROLLBACK INTO users SELECT \"users_backup\".\"id\", \"users_backup\".\"name\" FROM "
                       "'users_backup'";
        }
    }
