...
inline constexpr auto err_fatal_error_ptr = &ERR_FATAL_ERROR;
inline constexpr auto clamp_int_ptr = &std::clamp<int>;
...
TEST_CASE("generalized scalar udf") {
    ...
    SECTION("freestanding function") {
        constexpr auto err_fatal_error_f = "ERR_FATAL_ERROR"_scalar.quote(err_fatal_error_ptr);
    ...
    SECTION("specialized template function") {
        constexpr auto clamp_int_f = "clamp_int"_scalar.quote(clamp_int_ptr);
    ...
    SECTION("overloaded template function") {
        constexpr auto clamp_int_f =
            "clamp_int"_scalar.quote<const int&(const int&, const int&, const int&)>(clamp_int_ptr);
        storage.create_scalar_function<clamp_int_f>();
        {
            auto rows = storage.select(clamp_int_f(0, 1, 1));
            decltype(rows) expected{true};
    ...
    SECTION("escaped function identifier") {
        constexpr auto clamp_f = R"("clamp int")"_scalar.quote(clamp_int_ptr);
