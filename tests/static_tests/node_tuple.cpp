...
TEST_CASE("Node tuple") {
    ...
    //  simple
    static_assert(is_same<node_tuple<int>::type, std::tuple<int>>::value, "int");
    static_assert(is_same<node_tuple<float>::type, std::tuple<float>>::value, "float");
    
    {//  binary_condition
        using namespace conditions;
        
        static_assert(is_same<node_tuple<lesser_than_t<int, float>>::type, std::tuple<int, float>>::value, "lesser_than_t");
        static_assert(is_same<node_tuple<lesser_or_equal_t<int, float>>::type, std::tuple<int, float>>::value, "lesser_or_equal_t");
        static_assert(is_same<node_tuple<greater_than_t<int, float>>::type, std::tuple<int, float>>::value, "greater_than_t");
        static_assert(is_same<node_tuple<greater_or_equal_t<int, float>>::type, std::tuple<int, float>>::value, "greater_or_equal_t");
        static_assert(is_same<node_tuple<is_equal_t<int, float>>::type, std::tuple<int, float>>::value, "is_equal_t");
        static_assert(is_same<node_tuple<is_not_equal_t<int, float>>::type, std::tuple<int, float>>::value, "is_not_equal_t");
        static_assert(is_same<node_tuple<and_condition_t<bool, int>>::type, std::tuple<bool, int>>::value, "and_condition_t");
        static_assert(is_same<node_tuple<or_condition_t<bool, int>>::type, std::tuple<bool, int>>::value, "or_condition_t");
    }
    {//  binary_operator
        ...
        using DivTuple = node_tuple<div_t<int, float>>::type;
    ...
    {//  compound operator...
        ...
        {// union_all
            ...
            using Union = decltype(un);
            using Tuple = node_tuple<Union>::type;
            using Expected = std::tuple<decltype(&User::id), decltype(&User::name), const char *, decltype(&User::id), std::string, decltype(&User::name)>;
        ...
        {// except
            ...
            using Union = decltype(un);
            using Tuple = node_tuple<Union>::type;
            using Expected = std::tuple<decltype(&User::id), decltype(&User::name), decltype(&User::id), long, decltype(&User::id), decltype(&User::name), decltype(&User::id), long>;
    ...
    {// select
        ...
        {// select(columns(&User::id, &User::name), where(greater_or_equal(&User::id, 10) and lesser_or_equal(&User::id, 20)))
            auto sel = select(columns(&User::id, &User::name), where(greater_or_equal(&User::id, 10) and lesser_or_equal(&User::id, 20)));
            using Sel = decltype(sel);
            using Tuple = node_tuple<Sel>::type;
            using Expected = std::tuple<decltype(&User::id), decltype(&User::name), decltype(&User::id), int, decltype(&User::id), int>;
    ...
    {// get_all_t
        ...
        {// get_all<User>(where(is_equal(5.0, &User::id)))
            ...
            using GetAll = decltype(getAll);
            using Tuple = node_tuple<GetAll>::type;
            static_assert(is_same<Tuple, std::tuple<double, decltype(&User::id)>>::value, "get_all<User>(where(is_equal(5.0, &User::id)))");
        }
        {// get_all<User>(where(is_equal(5.0, &User::id)))
    ...
    {// having_t
        ...
        static_assert(is_same<Tuple, std::tuple<decltype(&User::id), int>>::value, "having(greater_or_equal(&User::id, 10))");
    }
    {// cast_t
    ...
    {// core_function_t
        ...
        {// date
            auto f = date(std::string("now"), std::string("start of month"), std::string("+1 month"), std::string("-1 day"));
            using Fun = decltype(f);
            using Tuple = node_tuple<Fun>::type;
    ...
    {// case
        auto c = case_<std::string>(&User::name)
