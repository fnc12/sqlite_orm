...
TEST_CASE("count_tuple") {
    ...
    }
    {
        auto t = std::make_tuple(where(lesser_than(&User::id, 10)),
                                 ...
                                 group_by(&User::name));
        using T = decltype(t);
        static_assert(count_tuple<T, conditions::is_where>::value == 2, "");
        static_assert(count_tuple<T, conditions::is_group_by>::value == 1, "");
        static_assert(count_tuple<T, conditions::is_order_by>::value == 0, "");
        static_assert(count_tuple<T, conditions::is_limit>::value == 0, "");
    }
    {
        auto t = std::make_tuple(group_by(&User::name), limit(10, offset(5)));
        using T = decltype(t);
        static_assert(count_tuple<T, conditions::is_where>::value == 0, "");
        static_assert(count_tuple<T, conditions::is_group_by>::value == 1, "");
        static_assert(count_tuple<T, conditions::is_order_by>::value == 0, "");
        static_assert(count_tuple<T, conditions::is_limit>::value == 1, "");
    }
    {
        ...
        static_assert(count_tuple<T, conditions::is_where>::value == 1, "");
        static_assert(count_tuple<T, conditions::is_group_by>::value == 0, "");
        static_assert(count_tuple<T, conditions::is_order_by>::value == 2, "");
        static_assert(count_tuple<T, conditions::is_limit>::value == 0, "");
    }
    {
