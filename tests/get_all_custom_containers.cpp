...
TEST_CASE("get_all deque") {
    ...
    tester.testContainer<std::vector<std::unique_ptr<User>>>(storage.get_all_pointer<User, std::vector<std::unique_ptr<User>>>());
    tester.testContainer<std::deque<std::unique_ptr<User>>>(storage.get_all_pointer<User, std::deque<std::unique_ptr<User>>>());
    tester.testContainer<std::list<std::unique_ptr<User>>>(storage.get_all_pointer<User, std::list<std::unique_ptr<User>>>());
    tester.testPreparedStatement<std::vector<std::unique_ptr<User>>>(storage, storage.prepare(get_all_pointer<User>()));
    tester.testPreparedStatement<std::vector<std::unique_ptr<User>>>(storage, storage.prepare(get_all_pointer<User, std::vector<std::unique_ptr<User>>>()));
    tester.testPreparedStatement<std::deque<std::unique_ptr<User>>>(storage, storage.prepare(get_all_pointer<User, std::deque<std::unique_ptr<User>>>()));
    tester.testPreparedStatement<std::list<std::unique_ptr<User>>>(storage, storage.prepare(get_all_pointer<User, std::list<std::unique_ptr<User>>>()));
}
