...
TEST_CASE("DbConnection") {
    ...
    auto oldStorage = make_storage(con,
        make_table("users",
    ...
    oldStorage.on_open = [&pDb](sqlite3* p) {
        pDb = p;         // must never be called because on_open_internal is only called once in DbConnection's constructor (since every further call uses an open shared connection)
    };
        ...
        register_migration(2, 3, [](const DbConnection& con) {
            ...
            for (auto& oldUser : allOldUsers) {
                ...
                if (spaceIndex != oldUserName.npos) { // space is found
                    ...
                    auto lastName = oldUserName.substr(spaceIndex, oldUserName.size() -
                        spaceIndex);
