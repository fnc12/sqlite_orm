...
TEST_CASE("statement_serializer column") {
    ...
    SECTION("with types and constraints") {
        ...
        SECTION("id INTEGER (implicit) NOT NULL") {
            ...
            expected = R"("id" INTEGER NOT NULL)";
        }
        ...
        SECTION("default") {
            SECTION("current") {
                SECTION("time") {
                    ...
                    value = serialize(column, context);
                    expected = R"("name" TEXT DEFAULT (CURRENT_TIME) NOT NULL)";
                }
                SECTION("date") {
                    auto column = make_column("name", &User::name, default_value(current_date()));
                    value = serialize(column, context);
                    expected = R"("name" TEXT DEFAULT (CURRENT_DATE) NOT NULL)";
                }
                SECTION("timestamp") {
                    auto column = make_column("name", &User::name, default_value(current_timestamp()));
                    value = serialize(column, context);
                    expected = R"("name" TEXT DEFAULT (CURRENT_TIMESTAMP) NOT NULL)";
                }
            }
        }
    }
    SECTION("without types and implicit constraints") {
        context.omit_column_type = true;
        SECTION("id INTEGER PRIMARY KEY") {
            auto column = make_column("id", &User::id, primary_key());
            value = serialize(column, context);
            expected = R"("id" PRIMARY KEY)";
        }
