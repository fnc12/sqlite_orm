    ...
    SECTION("columns") {
        ...
        SECTION("asterisk") {
            ...
            SECTION("mapped, implicit select order") {
                auto expression = select(asterisk<User>(selected_order::implicit));
                expression.highest_level = true;
                stringValue = serialize(expression, context);
                expected = R"(SELECT * FROM "users")";
            }
            SECTION("mapped, defined select order") {
                auto expression = select(asterisk<User>(selected_order::defined));
                expression.highest_level = true;
                stringValue = serialize(expression, context);
            ...
            SECTION("alias, defined select order") {
                ...
                auto expression = select(asterisk<als_u>(selected_order::defined));
                expression.highest_level = true;
                stringValue = serialize(expression, context);
            ...
            SECTION("object, defined select order") {
                auto expression = select(object<User>(selected_order::defined));
                expression.highest_level = true;
                stringValue = serialize(expression, context);
                expected = R"(SELECT "id", "name" FROM "users")";
            }
